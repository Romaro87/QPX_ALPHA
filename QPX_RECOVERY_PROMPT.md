...
- **VERIFIED_REPO:** Existing PR50 runtime is preserved. The supervised service is repointed to fresh runtime `runtime/qpx_volume_confirmation_25_iex_forward_research_paper_clean_v2` and profile environment `QPX_PAPER_PROFILE`; no broker order path is enabled and IEX remains explicitly non-SIP-parity research evidence.
...
- **NEXT EXACT ACTION:** Stage explicit source, profile, service, test, and continuity paths; inspect staged diff; commit and push `main`; install/reload/restart the supervised service; verify fresh runtime heartbeat and state identity.
