...
class ProfitSourceLedger:
 ...
 def record(self,c,config):
  ...
  if c.event_sequence<=self.last_state_event_sequence:raise ValueError("OUT_OF_ORDER_REALIZED_EVENT")
  ...
  recyclable=min(recyclable,max(0.,c.ordinary_investable_cash-self.recycled_profit_balance-self.withheld_profit_balance));withheld=max(0.,eligible-recyclable) if config.enabled else 0.;core={"source_event_id":c.realized_event_id,"source_event_sequence":c.event_sequence,"realized_timestamp":c.decision_timestamp.isoformat(),"gross_swing_profit":max(0,c.gross_realized_pnl),"tax_reserved":c.tax_reserved,"eligible_after_tax_amount":eligible,"configuration_fingerprint":config.fingerprint,"recycling_fraction":config.recycling_fraction,"recyclable_amount":recyclable,"withheld_until_rebalance_amount":withheld,"eligibility_event_sequence":c.event_sequence+config.redeployment_delay,"remaining_recyclable_amount":recyclable,"amount_consumed":0.,"amount_released_at_rebalance":0.,"status":"PENDING" if eligible and config.enabled else "INELIGIBLE"};lot=ProfitLot(**core,lot_id=fingerprint(core));self.profit_lots.append(lot);self.recycled_profit_balance+=recyclable;self.withheld_profit_balance+=withheld;self.ordinary_unrestricted_swing_cash=c.ordinary_investable_cash;self.processed_source_event_ids.add(c.realized_event_id);self.last_source_event_sequence=c.event_sequence;self.last_state_event_sequence=c.event_sequence;return lot
 ...
 def on_sleeve_rebalance(self,authoritative_cash,event_sequence):
  if type(event_sequence) is not int or event_sequence<=self.last_state_event_sequence:raise ValueError("OUT_OF_ORDER_SLEEVE_REBALANCE_EVENT")
  ...
  self.profit_lots=new;self.withheld_profit_balance=0.;self.recycled_profit_balance=0.;self.released_at_sleeve_rebalance+=released;self.ordinary_unrestricted_swing_cash=authoritative_cash;self.last_sleeve_rebalance_event_sequence=event_sequence;self.last_state_event_sequence=event_sequence;return released
 ...
 def from_dict(cls,p):
  ...
  expected_source=o.profit_lots[-1].source_event_sequence if o.profit_lots else 0;expected_state=max(expected_source,o.last_sleeve_rebalance_event_sequence)
  if o.processed_source_event_ids!={x.source_event_id for x in o.profit_lots} or o.last_source_event_sequence!=expected_source or o.last_state_event_sequence!=expected_state:raise ValueError("Corrupted ledger continuity")
  return o
