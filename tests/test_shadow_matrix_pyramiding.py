...
class ShadowPyramidingTests(unittest.TestCase):
 def test_exact_matrix_and_combinations(self):
  r=load_registry();self.assertEqual(len(r.configurations),33)
  ...
  for cap in (25,40,60,90):
   self.assertEqual({a.name:a.enabled for a in r.by_id[f"pyramid_{cap}"].accelerators},{"dynamic_sizing":False,"pyramiding":True,"capacity_arbitration":False})
   self.assertEqual({a.name:a.enabled for a in r.by_id[f"dynamic_pyramid_{cap}"].accelerators},{"dynamic_sizing":True,"pyramiding":True,"capacity_arbitration":False})
