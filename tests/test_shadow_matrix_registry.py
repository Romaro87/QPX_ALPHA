...
class ShadowMatrixRegistryTests(unittest.TestCase):
    ...
    def test_registry_contains_exactly_thirty_three_ordered_governance_identities(self) -> None:
        self.assertEqual(
            tuple(item.shadow_id for item in self.registry.configurations),
            EXPECTED_IDS,
        )
        self.assertEqual(len(self.registry.configurations), 45)
        self.assertNotEqual(
    ...
    def test_configuration_fingerprints_are_deterministic_and_unique(self) -> None:
        ...
        self.assertEqual(
            len({item.fingerprint for item in first.configurations}), 45
        )
